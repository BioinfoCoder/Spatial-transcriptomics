...
```{r UMAP_all}
matrix_data <- GetAssayData(seu.list[[i]], slot = "scale.data")
if (!is.null(matrix_data) && nrow(matrix_data) > 0) {
  matrix_data <- as.matrix(matrix_data)
  seu.list[[i]]@misc$kodama <- KODAMA::KODAMA.matrix(matrix_data)
  seu.list[[i]]@misc$kodama_vis <- KODAMA::KODAMA.visualization(matrix_data, method = "t-SNE")
} else {
  warning("Skipping KODAMA for sample ", names(seu.list)[i], ": No valid expression data.")
}
...
```{r UMAP_all}
# Loop through each sample in seu.list
for (i in seq_along(seu.list)) {
  cat("Processing sample:", names(seu.list)[i], "\n")

  # Ensure the object is Seurat before applying transformations
  if (!inherits(seu.list[[i]], "Seurat")) {
    next
  }

  # SCTransform with increased genes
  seu.list[[i]] <- SCTransform(seu.list[[i]], assay = "RNA", verbose = TRUE, n_genes = 3000, conserve.memory = FALSE)

  # PCA and Clustering
  seu.list[[i]] <- RunPCA(seu.list[[i]], assay = "SCT", verbose = TRUE)
  seu.list[[i]] <- FindNeighbors(seu.list[[i]], reduction = "pca", dims = 1:30, verbose = TRUE)
  seu.list[[i]] <- FindClusters(seu.list[[i]], resolution = 0.6, n.start = 20, verbose = TRUE)

  # Convert Seurat expression matrix for KODAMA
  matrix_data <- as.matrix(GetAssayData(seu.list[[i]], layer = "scale.data"))
  
  # Run KODAMA if the matrix is valid
  if (!is.null(matrix_data) && nrow(matrix_data) > 0) {
    seu.list[[i]]@misc$kodama <- KODAMA::KODAMA.matrix(matrix_data)
    seu.list[[i]]@misc$kodama_vis <- KODAMA::KODAMA.visualization(matrix_data, method = "UMAP")
  } else {
    warning("Skipping KODAMA for sample ", names(seu.list)[i], ": No valid expression data.")
  }
}
...
### QC 
***
install.packages("devtools")  # Install devtools if you haven't
devtools::install_github("author/KODAMAextra")  # Replace 'author' with the actual GitHub username/repo

data_matrix <- Embeddings(seu, reduction = "pca")
kodama_result <- KODAMA::KODAMA.matrix(data_matrix)

if (!requireNamespace("BiocManager", quietly = TRUE))
    install.packages("BiocManager")
BiocManager::install("KODAMAextra")
library(KODAMAextra)
# Run KODAMA matrix computation
  seurat_objects[[i]] <- RunKODAMAmatrix(seurat_objects[[i]],
                                         FUN = "fastpls",
                                         landmarks = 100000, splitting = 100,
                                         ncomp = 50, n.cores = 3)
  
  # Run KODAMA visualization
  seurat_objects[[i]] <- RunKODAMAvisualization(seurat_objects[[i]], method = "t-SNE")
}

saveRDS(seurat_objects, "seurat_objects.rds")

cc <- KODAMA::KODAMA.visualization(kodama_result, method = "t-SNE")
library(Seurat)
seu[["kodama"]] <- CreateDimReducObject(embeddings = cc,
                                           key = "KODAMA_",
                                           assay = DefaultAssay(seu))
kodama_data <- data.frame(
  KODAMA_1 = cc[, 1],
  KODAMA_2 = cc[, 2],
  label = seu@meta.data$ids
)


pdf("kodama_plot.pdf")  # Saves the plot as a PDF
ggplot(kodama_data, aes(x = KODAMA_1, y = KODAMA_2, color = label)) +
  geom_point(size = 3) +
  theme_minimal() +
