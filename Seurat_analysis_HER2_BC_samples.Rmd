...
meta_data <- read.csv("~/Downloads/her2st-master/data/clinical_data/meta_data.csv")
...
infoTable$spotfiles <- list.files(path = "../data/ST-spotfiles", full.names = T)[1:12]
head(infoTable)
# Assuming the patient ID column is called 'patient_id'
patient_data <- infoTable[infoTable$patient_id == "B", ]
...
<br>
```{r cca_list}
# Custom function to process patient data

seu.list <- lapply(unique(patient_data$patient_id), function(s) {
    InputFromTable(infotable = subset(patient_data, patient_id == s), 
                      min.gene.spots = 20,
...
total.qc <- do.call(rbind, lapply(seu.list, function(se) {
  data.frame(
    total_UMIs = sum(GetAssayData(se, slot = "counts")), 
    nSpots = ncol(se)
  )
}))
...
<br>
```{r all_samples_se}
# Increase the memory limit to a higher value if necessary
options(future.globals.maxSize = 1500 * 1024^2)  # Set to 1500 MiB (adjust as needed)

# Free up memory used by large objects before starting
gc()

# Adjust control for iteration limit
control <- list(maxit = 1000)  # Increase max iteration limit

# Step 1: Input data and create Seurat object
object <- InputFromTable(infotable = patient_data, 
                         min.gene.spots = 50,
                         min.spot.feature.count = 300, 
                         platform = "1k")

# Step 2: Take a random subset of cells (e.g., 10% of the total cells)
# Make sure the number of cells is within reasonable limits to prevent memory overflow
object <- SketchData(object, ncells = min(10000, ncol(object)), method = "LeverageScore", sketched.assay = "sketch")

# Step 3: Set the default assay and preprocess the data
DefaultAssay(object) <- "sketch"
object <- FindVariableFeatures(object)
object <- NormalizeData(object)

# Use a smaller dataset or batch operations to avoid memory overload
object <- ScaleData(object)
library(tidyverse)

# Avoid holding large intermediate results if not needed
object <- object %>% SCTransform()

# Run PCA on the subset, breaking the operation into manageable chunks if necessary
object <- RunPCA(object)

# Monitor memory usage and free up memory between heavy operations
gc()
...
object <- SetIdent(object, value = "ids")
object <- RunUMAP(object, reduction = "pca", dims = 1:20)
# Option 1: Using RColorBrewer for distinct colors
library(RColorBrewer)
distinct_colors <- brewer.pal(n = length(unique(object$seurat_clusters)), name = "Set3")

DimPlot(object, label = T)
str(object)
object <- ProjectData(object, assay = "SCT", full.reduction = "SCT.pca",
                      sketched.assay = "sketch", sketched.reduction = "pca",
                      umap.model = "umap", dims = 1:20,
...
qcMat <- do.call(rbind, lapply(1:length(seu.list), function(i) {
    object <- seu.list[[i]]
    do.call(rbind, lapply(unique(object[["ids", drop = TRUE]]), function(id) {
        # Use GetAssayData to retrieve count data
        repMat <- GetAssayData(object, assay = "RNA", slot = "counts")[, object[["ids", drop = TRUE]] == id]
        
        # Calculate the UMI and gene counts
        nUMI <- Matrix::colSums(repMat)
        nGene <- apply(repMat, 2, function(x) sum(x > 0))
        
        # Create the data frame with QC metrics
...
# Determine the number of Seurat objects
num_seus <- length(seu.list)

# Calculate the midpoint to select half of the objects
midpoint <- floor(num_seus / 2)

# Select the first half of the Seurat objects
seu_half <- seu.list[1:midpoint]

# Transform each selected Seurat object with error handling and progress reporting
seu_half_transformed <- lapply(seu_half, function(seu) {
  tryCatch({
    # Apply SCTransform to the Seurat object
    transformed_seu <- SCTransform(seu, 
                                    vars.to.regress = c("ids"), 
                                    return.only.var.genes = FALSE, 
                                    variable.features.n = NULL, 
                                    variable.features.rv.th = 1.1)
    return(transformed_seu)  # Return the transformed Seurat object
  }, error = function(e) {
    return(seu)  # Return the original Seurat object in case of an error
  })
})
...
library(KODAMAextra)
library(ggplot2)
data_matrix <- Embeddings(object, reduction = "pca")[, 1:20]  # PCA embeddings (first 20 dimensions)
kodama_result <- KODAMA::KODAMA.matrix(data_matrix)
cc <- KODAMA::KODAMA.visualization(kodama_result, method="UMAP")
object[["kodama"]] <- CreateDimReducObject(embeddings = cc,
                                           key = "KODAMA_",
                                           assay = DefaultAssay(object))
kodama_data <- data.frame(First_Dimension = cc[, 1],
                          Second_Dimension = cc[, 2],
                          label = object@meta.data$ids)

ggplot(kodama_data, aes(x = First_Dimension, y = Second_Dimension, color = label)) +
  geom_point(size = 3) +
  theme_minimal() +
  labs(title = "Kodama Dimensionality Reduction",
       x = "First Dimension",
       y = "Second Dimension",
       color = "Label")  # Adjusting this to provide a legend title
...
p1 <- ST.FeaturePlot(seu.list[[1]], ring.genes[1:2], palette = "RdBu", grid.ncol = 2, pt.size = 1.5, slot = "scale.data", center.zero = T, show.sb = F)
p2 <- ST.FeaturePlot(seu.list[[2]], ring.genes[1:2], palette = "RdBu", grid.ncol = 2, pt.size = 1.5, slot = "scale.data", center.zero = T, show.sb = F)
...
```For clustering, we'll use the `FindNeighbors` and `FindClusters` functions from Seurat, using the NMF vectors as input and a resolution of 0.4. For each patient, we subset the NMF matrix to include only those vectors that wew manually selected and stored into
<br>
